...
async function openBotCanonicalChat(name, pinned) {
  ...
  try {
    // include_hidden: Bot Mode sessions are always hidden from the global
    // sidebar, so the canonical-chat recovery scan must see them or every
    // launch would mint a fresh Bot Chat. Older gateways ignore the param.
