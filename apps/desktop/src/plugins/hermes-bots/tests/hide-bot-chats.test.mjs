...
test('the Bots session browser and canonical recovery list with include_hidden', () => {
  // Both session.list consumers that must see the always-hidden rows.
  assert.match(source, /session\.list', \{ profile: name, limit: 100, include_hidden: true \}/)
